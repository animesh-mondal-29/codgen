...
export const defaultProviderSettings = {
	andai: {
		endpoint: 'https://api.andaihub.ai',
		apiKey: '',
	},
	anthropic: {
		apiKey: '',
...
export const defaultModelsOfProvider = {
	andai: [ // https://docs.andaihub.ai/docs/models
		'gpt-4.1',
		'o3',
		// 'o3-mini',
		// 'o4-mini',
...
// ---------------- ANDAI  ----------------
const andAIModelOptions = { // https://platform.openai.com/docs/pricing
	'o3': {
		contextWindow: 1_047_576,
		reservedOutputTokenSpace: 32_768,
		cost: { input: 10.00, output: 40.00, cache_read: 2.50 },
...
const andaiSettings: VoidStaticProviderInfo = {
	providerReasoningIOSettings: {
		input: {
			includeInPayload: (reasoningInfo) => {
				if (!reasoningInfo?.isReasoningEnabled) return null
	...
	modelOptions: andAIModelOptions,
	modelOptionsFallback: (modelName) => {
		const lower = modelName.toLowerCase()
		let fallbackName: keyof typeof andAIModelOptions | null = null
		// if (lower.includes('o1') || lower.includes('o1')) fallbackName = 'o1'
		// if (lower.includes('o1-mini') || lower.includes('o1-mini')) fallbackName = 'o1-mini'
		...
		// if (lower.includes('gpt-4o')) { fallbackName = 'gpt-4o' }
		// if (lower.includes('gpt-4o-mini')) { fallbackName = 'gpt-4o-mini' }
		// if (lower.includes('gpt-4.1')) { fallbackName = 'gpt-4.1' }
		...
		// if (lower.includes('claude-3-opus')) fallbackName = 'claude-3-opus-20240229'
		// if (lower.includes('claude-3-sonnet')) fallbackName = 'claude-3-sonnet-20240229'
		if (fallbackName) return { modelName: fallbackName, recognizedModelName: fallbackName, ...andAIModelOptions[fallbackName] }
		return null
	},
}
